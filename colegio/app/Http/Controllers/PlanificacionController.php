...
class PlanificacionController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('evaluaciones.planificacion')
    ...
    public function store(Request $request)
    {
        ...
        if (sizeof($planificaciones) == sizeof($request->ins)) {
          for ($i=0; $i < sizeof($planificaciones) ; $i++) {
            ...
            $time = strtotime($request->fecha[$i]);
            $newformat = date('Y-m-d',$time);
            $planificaciones[$i]->pla_fecha   = $newformat;
            $planificaciones[$i]->pla_por     = $request->por[$i];
            $planificaciones[$i]->pla_des     = $request->des[$i];
        ...
        if (sizeof($planificaciones) < sizeof($request->ins)) {
          for ($i=0; $i < sizeof($request->ins) ; $i++) {
            if ($i < sizeof($planificaciones)) {
              ...
              $time = strtotime($request->fecha[$i]);
              $newformat = date('Y-m-d',$time);
              $planificacion->pla_fecha   = $newformat;
              $planificacion->pla_por     = $request->por[$i];
              $planificacion->pla_des     = $request->des[$i];
              $planificacion->save();
