...
@section('breadcrumb')
  <ol class="breadcrumb">
    ...
    <li>Admistrativo</li>
    <li class="active">Editar Depositos {{ $colegio[0]->nombre }}</li>
  </ol>
@endsection
@section('content')
  @if ($paso == 1)
  ...
  <form method="POST" action="{{ route('editardep.colegio') }}" accept-charset="UTF-8">
    {{ csrf_field() }}
...
@section('js')
  <script>
    function cargar(){
      $.get("editardepositocol/table/"+document.getElementById("ref").value+"",function(response,state){
        if (response.falla == 1) {
